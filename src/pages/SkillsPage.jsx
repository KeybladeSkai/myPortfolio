import { Libraries, skills } from "../data/Skills.js";
        ...
        {skills.map((skill, index) => (
          <ol key={index} className="flex gap-2 list">
            <li className="flex gap-2">
              <skill.logo className="text-[40px] font-normal" />
              <span className="text-[40px] font-normal">{skill.skill}</span>
            </li>
          </ol>
        ))}
      </div>
      ...
      <div className="px-2 pt-6 min-w-[300px] h-auto flex flex-col md:flex-row gap-6 flex-wrap justify-center mt-[40px] font-medium md:mt-[50px]">
        {Libraries.map((item, index) => (
          <ol key={index} className="flex gap-2 ">
            <li className="flex gap-2">
              <item.logo className="text-[40px]  font-normal" />
              <span className="text-[40px]  font-normal">{item.skill}</span>
            </li>
          </ol>
        ))}
      </div>
        ...
        {Tools.map((tool, index) => (
          <ol key={index} className="flex gap-2 ">
            <li className="flex gap-2 font-medium">
              <tool.logo className="text-[40px] font-normal" />
              <span className="text-[40px] font-normal">{tool.skill}</span>
            </li>
          </ol>
        ))}
      </div>
